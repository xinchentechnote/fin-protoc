...
func TestLuaBasicTypeMap(t *testing.T) {
	tests := []struct {
		typeName string
		expected LuaType
	}{
		{
			typeName: "u8",
			expected: LuaType{Name: "u8", To: "uint32", Size: 1},
		},
		{
			typeName: "u16",
			expected: LuaType{Name: "u16", To: "uint32", Size: 2},
		},
		{
			typeName: "u32",
			expected: LuaType{Name: "u32", To: "uint32", Size: 4},
		},
		{
			typeName: "u64",
			expected: LuaType{Name: "u64", To: "uint64", Size: 8},
		},
		{
			typeName: "i8",
			expected: LuaType{Name: "i8", To: "int", Size: 1},
		},
		{
			typeName: "i16",
			expected: LuaType{Name: "i16", To: "int", Size: 2},
		},
		{
			typeName: "i32",
			expected: LuaType{Name: "i32", To: "int", Size: 4},
		},
		{
			typeName: "i64",
			expected: LuaType{Name: "i64", To: "int64", Size: 8},
		},
		{
			typeName: "f32",
			expected: LuaType{Name: "f32", To: "float", Size: 4},
		},
		{
			typeName: "f64",
			expected: LuaType{Name: "f64", To: "double", Size: 8},
		},
		{
			typeName: "char",
			expected: LuaType{Name: "char", To: "char", Size: 1},
		},
	}

	for _, tt := range tests {
		t.Run(tt.typeName, func(t *testing.T) {
			result, ok := luaBasicTypeMap[tt.typeName]
			assert.True(t, ok)
			assert.Equal(t, tt.expected, result)
		})
	}
